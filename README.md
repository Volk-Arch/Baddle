...
Под капотом — 5 примитивов (`none / focus / AND / OR / XOR / bayes`) × тип цели (`finite / repeatable / open`) + стратегии. Один движок, 14 настроек. [Подробнее](docs/tick-design.md)
...
| **Байесовский мозг** | Confidence + Bayes + Байесовский режим | `P(H\|E) = P(E\|H)×P(H)/P(E)`. Режим Bayes: ввод гипотезы → наблюдения → live update. LLM оценивает prior и strength |
...
Baddle — не инструмент, а **второй мозг**, синхронизированный с первым через три контура:

```
1. Информационный (работает):   prediction error → precision → params → идеи → confidence → surprise → ⟲
2. Физиологический (планируется): HRV → θ/φ → ширина/поворот конуса → ⟲
3. Диалоговый (планируется):     ассистент → вопрос → ответ пользователя → нода → граф обновляется → ⟲
```

Первый контур замкнут автоматически. Второй замыкается через тело ([HRV](docs/hrv-design.md)). Третий — через диалог: LLM задаёт уточняющие вопросы, каждый ответ обогащает граф. Во время autorun — пауза когда системе нужна информация от человека.

Три канала управления конусом, одна система.
