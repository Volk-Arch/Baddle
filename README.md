...
- **Энтропия узлов** — обводка по уверенности модели (зелёный → красный), heatmap по токенам в detail panel при клике на узел
- **→ Flow** — directed flow layout: узлы в колонках по глубине (Topic→Think→Expand→Elaborate). Тупиковые ветки затухают. Переключатель свободный граф / поток
...
- **✂ Select & Collapse** — ручной коллапс: выбираешь произвольные узлы кликами, потом схлопываешь в один
