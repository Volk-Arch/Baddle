...
> Концептуальная вертикаль. Слои помечены статусом: **✅ работает**, **⚠ частично**, **🔮 дизайн (открыто)**.
> Детали → [TODO](TODO.md). Карта файлов → «Структура кода» ниже.

```
┌─────────────────────────────────────────────────────────────┐
│  МИР (v10) 🔮      браузер · календарь · погода · сенсоры   │
├─────────────────────────────────────────────────────────────┤
│  ТЕЛО (v5b) ⚠     HRV simulator ✅, Polar H10 🔮            │
├─────────────────────────────────────────────────────────────┤
│  ЧЕЛОВЕК (v9) ✅  chat · /graph/assist · decompose · ↯      │
│                    feedback 👍/👎/— cards, pause-on-question │
├─────────────────────────────────────────────────────────────┤
│  COGNITIVE LOOP ✅ непрерывный фоновый tick                  │
│  (v5a)             Horizon ↔ DMN/Scout переключение по NE    │
│                    логика возникает из зон d                 │
├─────────────────────────────────────────────────────────────┤
│  COGNITIVE STATE ✅ S · NE · DA_tonic/phasic · burnout       │
│  (v5d, унифицирован + precision · γ · τ_in/τ_out · T · policy │
│  с Horizon)        HRV → нейрохимия → параметры              │
│                    PROTECTIVE_FREEZE при burnout > θ          │
├─────────────────────┬───────────────────────────────────────┤
│  CONTENT GRAPH ✅   │  STATE GRAPH ✅ (v5e)                  │
│  идеи · evidence    │  history-as-graph                      │
│  embeddings-first   │  append-only state_graph.jsonl         │
│  (v8b) ✅           │  Git-аудит каждого шага                │
│  Camera (v8c) ✅    │  episodic query via distinct           │
├─────────────────────┴───────────────────────────────────────┤
│  NAND CORE ✅      d = distinct(A, B) ∈ [0, 1]               │
│  (v8a + v8d)       зоны: CONFIRM / EXPLORE / CONFLICT         │
│                    logit(post) = logit(prior) + γ·S·(1−2d)    │
├─────────────────────────────────────────────────────────────┤
│  WORKSPACES ✅     graphs/{id}/graph.json + state_graph.jsonl│
│  (v4)              cross-graph edges, meta-graph derived     │
├─────────────────────────────────────────────────────────────┤
...
**Детальные спеки по слоям:**
- Нейрохимия (S/NE/DA/burnout формулы) → [docs/neurochem-design.md](docs/neurochem-design.md)
- State-граф + Git-аудит → [docs/state-graph-design.md](docs/state-graph-design.md)
- CognitiveState (адаптивный контроллер) → [docs/horizon-design.md](docs/horizon-design.md)
- Tick cycle и NAND → [docs/tick-design.md](docs/tick-design.md), [docs/nand-architecture.md](docs/nand-architecture.md)
...
| [neurochem-design](docs/neurochem-design.md) | **S / NE / DA / burnout** — формулы, HRV-coupling, PROTECTIVE_FREEZE |
