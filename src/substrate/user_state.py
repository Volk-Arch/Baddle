...
После B5 (W0-W5) substrate всех state+dynamics — `src.substrate.rgk.РГК`.
UserState оставлен как тонкий facade для тестов и historical API: все методы —
1-line delegates на `_rgk.X`, properties читают `_rgk.user/system/etc.` напрямую.
...
Если class станет отбалластом — удалить вместе с `from .substrate.user_state
import …` в test fixtures.
"""
