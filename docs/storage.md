...
**state_graph.jsonl** — кто-что-когда системы. Источник истины для DMN walks, meta-tick `analyze_tail`, Markov transitions. Полный дизайн — [episodic-memory.md](episodic-memory.md).
...
**Lifecycle hook:** `/graph/add` с `node_type=goal` автоматически зовёт `add_goal()` и сохраняет `goal_id` в ноде. `nand.tick_emergent` при STOP CHECK когда goal resolved → `archive_solved()` + `complete_goal()`, маркирует нод `_goal_completed=True` (идемпотентность).
...
- [src/sensors/stream.py](../src/sensors/stream.py) — SensorStream + SensorReading, взвешенный агрегат, persist
- [src/sensors/adapters.py](../src/sensors/adapters.py) — адаптеры Polar / Apple
...
- [src/nand.py](../src/nand.py) — STOP CHECK → archive + complete
