...
Details → [user-model-design.md](user-model-design.md) секция 1c.
...
Details → [static-storage-design.md](static-storage-design.md).
...
Details → [user-model-design.md](user-model-design.md).
...
## Storage layout — data/ + graphs/<ws>/ + workspaces/

**Что.** Три слоя хранения с чёткой границей — person (singleton на
юзера, `data/`), workspace (per-проект, `graphs/<ws>/`), registry
(`workspaces/index.json` с cross_edges). Подробно → [storage-layout.md](storage-layout.md).

**Проверка структуры после первого запуска.**
```
data/
  settings.json, user_state.json, user_profile.json,
  goals.jsonl, activity.jsonl, checkins.jsonl, patterns.jsonl,
  plans.jsonl, roles.json, templates.json
graphs/main/
  graph.json, state_graph.jsonl, state_embeddings.jsonl, solved/
workspaces/
  index.json
```

**Проверка `/data/reset` endpoint.**
```
POST /data/reset  {"confirm":"RESET"}
  → {ok: true, removed_count: N, removed: [...]}
```
UI: Settings modal → «Danger zone» → 🗑 кнопка → prompt требует ввести
`RESET` → auto-reload. Настройки (settings.json), roles и templates
остаются (templates+roles пересоздаются из `src/defaults.py` на
следующем старте).

**Красный флаг.**
- После `/data/reset` без рестарта — старые ноды ещё висят в `/graph/info`
  → забыли сбросить runtime singleton (`reset_graph()` + `set_global_state`).
- Новый пользователь не видит `roles.json` в data/ после первого старта
  → `_ensure_defaults_seed()` в ui.py не сработал.
- При `/graph/switch` solved-архив предыдущего workspace'а виден в
  списке → `_iter_archive_files()` сканирует только активный, надо все.
