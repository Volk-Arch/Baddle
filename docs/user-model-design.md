...
вызывается автоматически после каждого объективного `update_from_*`
(HRV, feedback, энергия).
...
(только явные, объективные — без догадок по timing):

| Событие | Вклад | Обоснование |
|---------|-------|-------------|
| `accepted` feedback | +0.7 | явное «да», однозначный сигнал |
| `rejected` feedback | −0.7 | явное «нет» |
| streak rejected > accepted +3 | доп −0.05 × overage | серия неодобрений |

**Убрано 2026-04-20** (шумные feeders, не коррелировали с valence):
- ~~Быстрый ввод < 30с → +0.2~~ (quick ≠ интерес, часто нервный ввод)
- ~~Пауза > 5 мин → −0.2~~ (пауза ≠ неприятно, чаще просто работа/кофе)
