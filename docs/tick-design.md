...
Конус описывает форму внимания, Friston-loop — что его двигает. **Тик** — один атомарный шаг этой механики: сгенерируй / объедини / углуби / проверь. Какой именно — решает **когнитивное состояние** (CognitiveState), которое читает текущее распределение PE-каналов и выбирает следующую фазу работы с графом. CognitiveState — контроллер конуса; тик — его такт. Horizon, описанный в следующей главе, разворачивает этот контроллер в деталях.
...
**Навигация:** [← Friston loop](friston-loop.md) · [Индекс](README.md) · [Следующее: Horizon →](horizon-design.md)
