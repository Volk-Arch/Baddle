...
| `_check_dmn_cross_graph` | 60 мин | Cross-workspace bridges (serendipity между work/personal/...) | `DMN_CROSS_GRAPH_INTERVAL` |
...
## DMN cross-graph — serendipity между workspaces

Раз в час, ищет мосты **между разными workspaces** (work / personal /
research / ...). Используется та же pump-механика, но source и target
берутся из разных content graphs.

Результат → alert `dmn_cross_graph` («между work и personal нашлась
связь: ...»). Это **serendipity канал** — тема в одном контексте
резонирует с темой в другом.

Подробно — [cross-graph-design.md](cross-graph-design.md) (continuity +
seed nodes при switch).
...
Baddle **не спит** между запросами. Даже когда юзер закрыл вкладку:
каждые 10 мин пытается найти мост между далёкими идеями, каждые 30 мин
углубляет открытую цель, каждый час доводит граф до STABLE или ищет
cross-workspace связь, раз в сутки переваривает эмоционально-насыщенные
эпизоды и чистит слабое.
...
**Навигация:** [← Thinking operations](thinking-operations.md) · [Индекс](README.md) · [Cross-graph →](cross-graph-design.md)
