...
[consolidation.py](../src/consolidation.py) делает прунинг слабых и архив, время последнего доступа обновляется при каждом обращении. Затухание confidence у неиспользуемых нод вводится отдельным шагом в ночном цикле; константы подбираются под реальное использование.
...
- **6-я ось** — `UserState.agency` (VAD-Dominance) собирается; через 2-3 мес use смотрим, нужна ли SDT meaning/relatedness как 6-я ось ([TODO § OQ #2](../planning/TODO.md)).
- **ACh/GABA proxy quality** — реализованы как proxy от формы графа ([neurochem-design § Источники сигнала](neurochem-design.md)); калибровка по distribution `balance()` через 2 мес ([TODO § Calibration](../planning/TODO.md)).
