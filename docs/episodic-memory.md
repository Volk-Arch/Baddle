...
- `src/meta_tick.py` — `analyze_tail`, `apply_policy_nudge`
- `src/consolidation.py` — `decay_unused_nodes`, `consolidate_content_graph`,
  `consolidate_state_graph`, `consolidate_all`
- `src/graph_logic.py` — `touch_node`, `touch_nodes` (hebbian boost)
- `src/tick_nand.py` — `analyze_tail` в ASK check, `_emit()` добавляет
  state-ноды
...
(см. [OQ #1](../planning/open-questions.md)), multi-parent state_graph для
CONFLICT-ветвления, DMN walks по state-графу.

---

**Навигация:** [← Friston loop](friston-loop.md) · [Индекс](README.md) · [Следующее: Static storage →](static-storage-design.md)
