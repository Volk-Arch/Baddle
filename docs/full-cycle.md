...
- **Состояние системы** (`_rgk.system` Resonator) — дофамин (gain) от различия (d) в тике, серотонин (hyst) от стабильности изменений весов (ΔW), норадреналин (aperture) от энтропии распределения весов, ацетилхолин (plasticity) от частоты добавления нод и качества DMN-мостов, ГАМК (damping) от freeze-flag и scattering-proxy. Pressure layer (`_rgk.conflict/silence_press/imbalance_press`) — накопитель конфликтов от d·(1−серотонин) и др. Недавняя ошибка предсказания награды (RPE) от модуля разности posterior и prior, зрелость растёт по verifieds.

Плюс: **рассогласование** (sync_error) = ‖`_rgk.user.vector()` − `_rgk.system.vector()`‖ в 5D chem-пространстве (DA/5HT/NE/ACh/GABA), **граф контента** с embedding-ами и матрицей различий в тике (зоны NAND: согласие / исследование / конфликт), **цикл когнитивной петли** (DMN 10 минут, state_walk 20 минут, ночной цикл 24 часа, briefing 24 часа, foreground-тик при `/graph/tick`), **state_graph** как append-only git-аудит (каждый тик → запись), **meta-tick** на хвосте из 20 последних состояний (паттерны stuck / rejection / rpe / monotony).
...
Одна метрика меряет всё: **рассогласование** (sync_error) = ‖`_rgk.user.vector()` − `_rgk.system.vector()`‖ в 5D chem-пространстве (DA/5HT/NE/ACh/GABA). Per-axis breakdown — `sync_error_wave`, диагностика «по которой частоте расхождение». Фича оценивается по одному критерию — снижает ли она рассинхрон с конкретным пользователем.
