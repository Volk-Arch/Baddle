...
| Модель | Размер (Q4) | Статус | Комментарий |
|--------|-------------|--------|-------------|
| **Qwen3-8B** | ~5 GB | ✅ протестирована | Основной выбор. Отлично следует коротким инструкциям, стабильная генерация |
| Qwen3.5-9B | ~6 GB | ⚠️ проблемы | Более "разговорчивая" — начинает рассуждать вместо выдачи идеи, эхоит промпт |
| Qwen3-14B | ~9 GB | не тестирована | Если хватает VRAM |
| Llama-3.1-8B | ~5 GB | не тестирована | Альтернатива Qwen |

> **Рекомендация:** Qwen3-8B (Q4_K_M). Протестирована, работает стабильно с циклом generate→merge→elaborate→doubt.
