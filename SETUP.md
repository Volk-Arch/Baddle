...
### Graph (основной)
Граф мыслей с автоматическим мышлением — задаёшь цель, нажимаешь Run, система
думает сама. См. [README](README.md) для концепции.

### Chat
Обычный чат с моделью. История сохраняется в сессии. Кнопка **Continue**
догенерирует ответ если он обрезался.

---

## Параметры генерации

Настраиваются прямо в интерфейсе, для каждого режима свои.

### Температура (temp)

| Значение | Эффект |
|---|---|
| **0.0** | всегда самый вероятный токен (greedy) |
| **0.3–0.7** | баланс предсказуемости и разнообразия |
| **0.9** | для творческих задач и брейнсторма |
| **1.2+** | сильная вариативность, иногда нестабильно |

### Max tokens

Лимит токенов на один ответ. В Graph-режиме для финального эссе настраивается
отдельно: **Essay tokens** (по умолчанию 6000, максимум 30000).
