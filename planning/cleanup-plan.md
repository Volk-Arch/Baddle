...
- **W16.1b** Phase-aware comparison — phase + amplitude per component (требует velocity tracking ∂axis/∂t). Реализуется когда понадобится диагностика «не просто разница, но в каких axis user **ускоряется** относительно system». ~1-2ч. Substrate готов (axes EMA уже есть), нужен буфер `_prev_axes` снимок прошлого тика.
