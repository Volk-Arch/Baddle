...
- **W14.7 cognitive_loop split** (~2-3ч) — `process/bookkeeping.py` + briefings extract из cognitive_loop. Файл всё ещё ~2670 LOC. естественное продолжение W14.6
...
- **W16.2 Analogy injection** (~3-3.5ч) → `src/transfer/analogies.py`. теперь разблокирован после W14.6
...
Источники истины:
- **`git log --oneline`** — what was committed when, с descriptive messages
- **`memory/project_session_*`** snapshots — narrative summaries по сессиям
  (включая context, lessons, design deviations)

Done log как отдельная таблица **удалена 2026-04-29** — дублировала git log
без added value (per `feedback_decisions_minimal` принцип). Если нужен
aggregated view закрытого — `git log --oneline --grep="^W"` достаточно.
...
W16.1a (amplitude per axis) + W16.1b (phase-aware) — `git log --grep=W16.1`. Spectral diagnosis: где расхождение + в каких axes user ускоряется относительно system.
...
### Design deviations — статус 2026-04-29

✅ **4 deviations документированы** в [workspace-design.md](workspace-design.md) inline через **«Реальность W14.X»** блоки. Дизайн теперь reflect'ит actual implementation; будущая сессия читает design = читает код.

⏳ **2 deviations остаются TODO** для W14.5+ (will close через **code update**, не design):

| # | Что сейчас | Что должно стать | Wave |
|---|---|---|---|
| 5 | Generic `synthesize_similar` (text concatenation) для всех kinds | Per-kind strategies: `pump.scout` для sync_seeking; `consolidation._collapse_cluster_to_node` для observation_suggestion; `SmartDC` для alerts | W14.5+ |
| 4b | Scout/dmn-bridge immediate `record_committed` | Если bridges станут потоком (background scout каждые 5 мин с регулярными hits) — переключить на `accumulate=True` path для cross-processing над bridges | W14.5+ или W14.7 |

См. **TODO для W14.5+** в `planning/workspace-design.md` §W14.5.

**Закрытые deviations (в design):**
1. `add(source, kind)` → `add(actor, action_kind)` — Action Memory taxonomy
2. `actor="baddle"` only → `actor: "baddle"|"user"` — user_chat тоже в workspace
3. Briefings `accumulate=True, ttl=3600` → `accumulate=False, immediate, ttl=24h/7d` — explicit POST publication
4. `commit()` пишет `chat_log.jsonl` → scope mutation only + parallel `chat_history.jsonl` UI layer
