...
> **⚠ Читать первым: [simplification-plan.md](simplification-plan.md).** Consolidation **Фазы A+B+C завершены** (2026-04-24/25): MetricRegistry + Signal Dispatcher + 13 детекторов + 3-zone capacity model. Cognitive_loop сжат на ~600 строк, правила 1-2 из §4 реализованы, dual-pool legacy полностью удалён. **Сейчас:** 2-недельное окно реального use → калибровка `compute_urgency` (Phase B) + `cognitive_load` коэффициентов (Phase C). **Следующее:** Tier 2 фичи (резонансные, RAG, sensors) — разморожены, берутся в очерёдности. Нарушение дисциплины = возврат к 20k строк.
...
## 🧠 Capacity (3-zone, после Фазы C 2026-04-25)

3-контурная модель из [docs/capacity-design.md](../docs/capacity-design.md)
(физио / эмо / когнитивный с capacity-zone как decision gate) — реализована
в Фазе C: `UserState.capacity_zone` derived property + `cognitive_load_today`
+ `day_summary` aggregate + 3 мини-индикатора 🟢🟡🔴 в UI.

**Открытые задачи поверх capacity:**

- [ ] **detect_evening_retro расширить** по 6 observable + progress дня
      (cognitive_load_today, tasks_started, tasks_completed, context_switches,
      complexity_sum, progress_delta).
- [ ] **detect_morning_briefing** (через `_build_morning_briefing_sections`)
      — добавить causal explanation зоны (какой контур провисает + почему).
- [ ] **/assist/simulate-day переписать** на прогноз capacity-зон (предсказать
      cognitive_load_today от плана активностей через cap=8/10/3 нормализацию).
      Endpoint удалён в Фазе C, нужна capacity-based реимплементация.
- [ ] **Калибровка коэффициентов** в `compute_cognitive_load`
      (0.20/0.30/0.30/0.25/0.25 веса) — крутить на 2-недельном окне данных.
- [ ] **Калибровка порогов** `CAPACITY_PHYS_COHERENCE_MIN`/etc по корреляции
      с sync_error. Через 2 нед use.
- [ ] **OQ #1 (personal capacity prior)** — Bayesian online EMA на индикаторы
      → per-user thresholds в `user_profile.json`.
