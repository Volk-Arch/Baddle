...
- [ ] **Resonance transfer (W16)** — direction для главного вопроса проекта (как нейросеть быстро попадает в узор нового юзера). ~~`sync_error_wave` per axis~~ ✅ 2026-04-28 (W16.1a + W16.1b — amplitude + phase). **W16.2 — analogy injection layer** (sync deficit detector → probe в чате, не Settings). Empty state probe ⭐ critical first contact (без goals — нечем sync'аться). Like/Dislike segmentation на 👎. **Зависит от W14 chat refactor** (промежуточное хранилище для сообщений). Spec — [cleanup-plan W16.2](cleanup-plan.md). **~3-3.5ч после W14**. W16.3 few-shot bias / W16.4 adiabatic / W16.5 outbound LLM reformulation — отдельные sub-waves. **Validation:** Testable claim 8 в rgk-spec — A/B test, differential ≥3x подтверждает.
